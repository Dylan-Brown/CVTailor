"""
sourcing/ats_boards.py

Sources job postings from Greenhouse, Lever, and Ashby's public JSON
APIs. Ported from the archived RemoteJobApplier project's
boards/ats_boards.py -- this was explicitly NOT the part that failed.
These are public, documented, unauthenticated JSON endpoints, not
scraped HTML: no bot detection, no Cloudflare challenge, no ToS gray
area the way LinkedIn/Indeed scraping was.

  Greenhouse: GET https://boards-api.greenhouse.io/v1/boards/{token}/jobs?content=true
  Lever:      GET https://api.lever.co/v0/postings/{slug}?mode=json
  Ashby:      GET https://api.ashbyhq.com/posting-api/job-board/{slug}?includeCompensation=true

What changed from the original: the RawJob/JobBoard class hierarchy is
gone (it existed to support the automated-submission pipeline this
doesn't need). These are now plain generator functions yielding dicts
already shaped to match CVTailor's JDInput schema, so output can feed
straight into `py_stage0_intake.py --jd-json` with zero adapter code.

Usage:

    from sourcing.ats_boards import fetch_all_target_companies

    for jd_input_dict in fetch_all_target_companies(
        keywords=["cloud architect", "solutions architect", "senior software engineer"],
        max_jobs_per_board=50,
    ):
        # jd_input_dict is ready to json.dump() and hand to stage0_intake
        ...
"""
