#!/usr/bin/env python3
"""
py_answer_question.py — answers a free-form application-form question
("Why are you interested in this role?", "Describe a time you...",
etc.) grounded in the claims ledger, the same source-of-truth
principle every other stage in this pipeline follows: no fact, number,
or claim in the answer that isn't traceable to something real on the
resume.

Unlike the cover letter/resume pipeline, this is deliberately a single
generation call, not a multi-stage verify/critique pipeline -- it's
meant for one-off, read-it-yourself-before-using-it answers to
application-form questions, not unattended batch processing across
many JDs. You're the review step here.

Usage:
    # Simplest -- answer from a resume variant alone, no company context
    python src/util/py_answer_question.py --question "Why are you interested in this role?" --variant Backend

    # With company/JD context (auto-detects variant from routing.json,
    # pulls in company_brief.json for company-specific framing)
    python src/util/py_answer_question.py --question "Why do you want to work here?" --app-id toast_senior_software_engineer_consumer_2026-08-07

    python src/util/py_answer_question.py --question "..." --app-id ... --out my_answer.txt
"""
import argparse
import sys
