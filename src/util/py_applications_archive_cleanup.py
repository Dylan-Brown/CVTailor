#!/usr/bin/env python3
"""
py_applications_archive_cleanup.py — reclaims space from applications you're
done processing (either genuinely sent, or test/practice runs) by
deleting the intermediate audit-trail files while keeping what
actually has lasting value.

KEPT, always:
  - generated_materials/           the actual resume/cover-letter/
                                    follow-up-steps output — this is
                                    what you sent (or would have sent)
  - jd_input.json /
    jd_input.processed.json        the job posting itself
  - review_decisions.json          your actual approve/reject/edit
                                    choices — the real record of what
                                    you decided, not what a model
                                    proposed
  - routing.json                   which resume variant was used

DELETED (all of this exists to help you review BEFORE sending; once
an app is archived, its debugging value has already been spent):
  - candidate_edits.json, verified_edits.json, critiqued_edits.json
  - verification_report.json, critique_report.json
  - critiqued_out.jsonl, discarded_edits.jsonl
  - edit_brief.json, company_brief.json, edit_log.jsonl
  - __pycache__/ and any other stray non-listed files

Defaults to --dry-run (lists what would be deleted and the space
reclaimed, deletes nothing) -- pass --force to actually delete.

Usage (run from jobs/ root):
    python src/util/py_applications_archive_cleanup.py                    # dry-run, default paths
    python src/util/py_applications_archive_cleanup.py --force             # actually delete
    python src/util/py_applications_archive_cleanup.py --root applications/archive --force
"""
import argparse
import sys
