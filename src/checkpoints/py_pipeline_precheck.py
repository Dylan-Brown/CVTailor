#!/usr/bin/env python3
"""
py_pipeline_precheck.py -- read-only sanity check (files present, Python
version, packages, provider reachable, inputs actually open) before you
run the real pipeline. No API calls unless you pass --check-api-live.
"""
import argparse
import importlib
import json
...
def check_batch_config(root: Path):
    ...
    try:
        ...
        check("config/resume_pdfs/ has at least one resume", True,
              f"{len(variants)} variant(s): {label}")
    except Exception as e:
        check("config/resume_pdfs/ has at least one resume", False, str(e))
        variants = {}
