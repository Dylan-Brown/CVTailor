#!/usr/bin/env python3
"""
py_post_pipeline_verify_materials.py — the automated half of the Definition of Done (see
DEFINITION_OF_DONE.md). Runs against an already-assembled application
(generated_materials/ must exist) and checks the concrete, mechanical
things a human skim tends to miss when tired, in a hurry, or trusting
that "it ran without errors" means "it's good."

This does NOT replace your own read of the actual letter -- it catches
the specific failure modes this project has actually hit (see each
check's docstring for the real incident that motivated it), not "is
this a good cover letter" in general. A clean pass here is necessary,
not sufficient.

Usage:
    python src/checkpoints/py_post_pipeline_verify_materials.py --app-id toast_senior_software_engineer_...
