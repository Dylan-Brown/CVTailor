#!/usr/bin/env python3
"""
py_pipeline_assemble.py — the assembly checkpoint, run after stage 8
review.

Scans applications/ for any app that has review_decisions.json (i.e.
you've run stage 8 for it — this script never touches anything that
hasn't been through your review) and hasn't been assembled yet. Runs
stage 9 (which writes the real resume/cover-letter docx straight into
applications/<app_id>/generated_materials/ — a dedicated subfolder,
kept separate from the dozen-plus audit-trail JSON files also living
in that app's folder), converts to PDF where possible, and — only on
full success — marks the JD processed by renaming jd_input.json ->
jd_input.processed.json in place.

If assembly fails partway through, jd_input.json stays unrenamed, so
nothing is silently lost track of — that's a deliberate choice: an
un-renamed jd_input.json means "still pending in some way,"
generated_materials/ actually having real output in it means "done."

Usage:
    python src/controls/py_pipeline_assemble.py [--force] [--keep-docx] [--dry-run]

    # regenerate PDF(s) only, from .docx already hand-edited after a
    # previous assemble -- skips stage 9 entirely so it never overwrites
    # those edits. Accepts one or more comma-separated app-id prefixes.
    python src/controls/py_pipeline_assemble.py --app-id valon
    python src/controls/py_pipeline_assemble.py --app-id valon,toast_senior,evolve
"""
import argparse
import json
import shutil
