#!/usr/bin/env python3
"""
py_stage02_ingest_cv_claims.py — resume/cover letter -> ClaimsLedger + StyleProfile

This is an EXTRACTION-ONLY pass. The prompt is deliberately narrow: pull
out what's already there, don't summarize, don't infer unstated
achievements, don't round numbers. Everything downstream trusts this
ledger as ground truth, so garbage in here is the one failure mode that
can't be caught by stage 6's verifier (it can only check against THIS).

Usage:
    python py_stage02_ingest_cv_claims.py \
        --resume /path/to/resume.pdf \
        --cover-letter-sample /path/to/sample_cover_letter.txt \
        --out-dir applications/acme_cloud_architect_2026-07-27 \
        [--dry-run]
"""
import argparse
import sys
