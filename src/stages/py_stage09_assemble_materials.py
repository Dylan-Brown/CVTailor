#!/usr/bin/env python3
"""
py_stage09_assemble_materials.py — apply approved/edited decisions to your resume
docx template (formatting-preserving, table-aware) and generate a real
cover letter docx, then log the whole application run.

Note on approach: python-docx here rather than the unzip-XML approach —
this script runs standalone on your machine with no LibreOffice/pandoc
dependency required, which matters more for a script that has to Just
Work on a Windows box than XML-level control does. If a specific
resume template ever fights this approach (heavily merged/split runs,
content controls, etc.), the fallback is unzip -> edit word/document.xml
directly -> rezip, same as any other docx surgery — but try this first.

NEW: interactive cover-letter review loop (see interactive_cover_letter_review()
below). Once stage 8's per-sentence decisions are assembled into the full
cover letter body, you get one more pass over the WHOLE text before it's
written to disk:
    (a)ccept  — proceed to build the docx as-is
    (m)anual  — opens the text in Notepad; edit, save, close Notepad,
                and the (possibly changed) text is picked back up
    (f)eedback — type a note describing what to change; the current text,
                the original baseline, and your note go to the Anthropic
                API, which returns a revised full letter staying close to
                the current text while addressing the feedback
Each of (m)/(f) loops back to the same review, so you can go several
rounds until you're satisfied. Always runs whenever there's cover
letter content -- no opt-out flag. Safe to try during --dry-run: the
loop itself never writes files; only the final docx build further down
is dry_run-gated.

Usage:
    python py_stage09_assemble_materials.py \
        --resume-template /path/to/Dylan_Brown_Resume_Final.docx \
        --review-decisions applications/.../review_decisions.json \
        --verified-edits applications/.../verified_edits.json \
        --out-dir applications/... \
        --company "Acme" --role-title "Cloud Architect" \
        --applicant-name "Dylan Brown" \
        [--applicant-email you@example.com] [--applicant-phone "215-555-0100"] \
        [--provider claude] \
        [--dry-run]
"""
import argparse
