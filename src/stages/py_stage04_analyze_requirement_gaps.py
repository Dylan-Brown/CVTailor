#!/usr/bin/env python3
"""
py_stage4_gap_analysis.py — diff the JD's requirements against the
claims ledger. Produces EditBrief: what's covered, what's missing, what
you're over-indexed on. Nothing gets rewritten here — this stage only
decides what stage 5 should pay attention to.

If jd_input.json has requirements_raw (the extension found a clearly
delimited requirements list), that's handed to the model directly
alongside the full text — more precise than making the model re-derive
a requirements list from prose every time. If requirements_raw is
empty, the model derives requirements from full_description_text same
as before; the stage degrades gracefully either way.

Usage:
    python py_stage4_gap_analysis.py \
