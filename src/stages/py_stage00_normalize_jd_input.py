#!/usr/bin/env python3
"""
py_stage00_normalize_jd_input.py — validation boundary between the Chrome extension
(untrusted: it's parsing arbitrary web pages) and the rest of the
pipeline (trusted: everything past this point assumes well-formed
input).

Auto-detects whether the input JSON is already in JDInput's shape or
is the Job Data Harvester extension's 13-key schema (JobTitle/Company/
WorkplaceType/...), and normalizes the latter before validating. Either
format works with the same --jd-json flag — no separate mode needed.

Does three things:
  1. Normalizes (if needed) and validates against JDInput — fails loud
     and specific if something's missing or the wrong type, rather than
     letting a malformed file surface as a confusing error three stages
     later.
  2. Canonicalizes it to applications/{app_id}/jd_input.json.
  3. Derives an app_id (company_role_date) if you didn't pass one.

Usage:
    python py_stage00_normalize_jd_input.py \
        --jd-json /path/to/downloaded_from_extension.json \
        [--source-url "https://..."] \
