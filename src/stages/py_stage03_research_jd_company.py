#!/usr/bin/env python3
"""
py_stage03_research_jd_company.py — structured research call against the Anthropic
API, scoped to THIS job description and THIS resume, not generic
company trivia.

Reads jd_input.json (produced by stage 0 from the Chrome extension's
output) rather than a raw text file — this gives the research prompt
access to structured fields (salary range, remote type, employment
type) in addition to the raw description text, so it doesn't have to
re-derive those from prose.

Uses the web_search tool so findings come with source_url citations —
those citations get carried forward so you can eyeball them at review
time instead of trusting a bare claim.

Usage:
    python py_stage03_research_jd_company.py \
