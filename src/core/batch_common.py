"""
batch_common.py — shared helpers for run_pipeline.py, py_batch_assemble.py,
and the src/checkpoints/, src/controls/ scripts.

Nothing pipeline-specific lives here that isn't about orchestration —
stage logic itself never moves out of src/stages/. This module knows
about the config/ folder convention, resume-variant routing, and the
applications/-centric queue model (applications/ IS the queue AND the
drop zone -- there's no separate intake folder anywhere. To queue a
new JD by hand: either drop the raw exported JSON straight into
applications/ (any filename) and it gets auto-organized into its own
applications/<name>/ subfolder, or pre-make applications/<name>/
yourself and drop the raw JSON inside it if you want to control the
folder name. Either way, the next run_pipeline.py run finds it
and runs stage 0 on it in place. See drain_raw_intake() in
run_pipeline.py.)
"""
...
def load_resume_variants() -> dict[str, ResumeVariant]:
    ...
    files = sorted(
        ...
        if p.is_file() and p.suffix.lower() == ".json" and not p.name.lower().endswith(".example.json")
    )
    if not files:
        raise FileNotFoundError(
            f"No resume variant config found in {RESUME_VARIANTS_DIR}/ -- copy one of the "
            f"*.example.json files there to a real .json file (e.g. backend.example.json -> "
            f"backend.json), point its \"resume\" field at your actual resume in {RESUME_DIR}/, "
            f"and repeat for each resume you want the pipeline to route between."
        )
